...
class TFRecordDataset:
    def __init__(self,
                 ...
                 label_file=None,
                 # 0 = no labels, 'full' = full labels, <int> = N first label components.
                 max_label_size=0,
                 repeat=True,     # Repeat dataset indefinitely.
                 # Shuffle data within specified window (megabytes), 0 = disable shuffling.
                 shuffle_mb=4096,
                 # Amount of data to prefetch (megabytes), 0 = disable prefetching.
                 prefetch_mb=2048,
        ...
        if self.label_file is None:
            guess = sorted(glob.glob(os.path.join(
                self.tfrecord_dir, '*.labels')))
            if len(guess):
